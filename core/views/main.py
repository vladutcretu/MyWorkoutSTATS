...
def main(request):
    ...
    # Calculate the date for the desired day based on days_diff value
    ...
    # If user not logged in, workout is None so display only basic info on page
    else:
...
def build_redirect_url(request, default_url=""):
    """
    Created for refactoring: after an action redirect users to the same page
    from which they initiated the action
    """
