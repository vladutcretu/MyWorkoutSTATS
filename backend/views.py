...
def collection_exercises(request):
   """View used to show muscle groups and exercises from WGER API: https://wger.de/en/software/api"""
   # Auth credentials
   ...
   exercises = fetch_WGER_exercises_data(username, password, musclegroup_id) if musclegroup_id else []
   
   context = {
      'musclegroups': musclegroups,
      'exercises': exercises
   }
   return render(request, 'backend/exercises_collection.html', context)
...
def get_WGER_token_and_auth(username, password):
   """Get the access token and make authentication to WGER API: https://wger.de/en/software/api"""
   ...
   if not access_token:
      return f"Access token can't be obtained."

   # Check token
   verify_response = requests.post(
      'https://wger.de/api/v2/token/verify',
      data = {'token': access_token}
   )

   if verify_response.status_code != 200:
      return f"Access token is invalid or expired: {verify_response.status_code}"
...
def fetch_WGER_musclegroups_data(username, password):
   ...
   if not access_token:
      return f"Access token is invalid or expired."
...
def get_WGER_musclegroups(data):
   ...
   return [(musclegroup.get('id'), musclegroup.get('name')) for musclegroup in data.get('results', [])]
...
def fetch_WGER_exercises_data(username, password, category):
   ...
   url = f'https://wger.de/api/v2/exercise/?category={category}&language=2&limit=20'
   headers = {'Authorization': f'Bearer {access_token}'}
...
def get_WGER_exercises(data):
   ...
   return [(exercise.get('name'), exercise.get('description')) for exercise in data.get('results', [])]