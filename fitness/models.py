...
class Workout(models.Model):
    ...
    public = models.CharField(
        max_length=3, choices=PUBLIC_CHOICES, default="no"
    )
    likes = models.ManyToManyField(
        CustomUser, related_name="workout_likes", blank=True
    )
    ...
    def __str__(self):
        return (
            f"Owner: {self.user} - Workout: {self.name} - Date: {self.created}"
        )
...
class WorkoutComment(models.Model):
    """
    Model for comments section on public workouts, allowing replies to other
    comments
    """

    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
    workout = models.ForeignKey(
        Workout, related_name="comments", on_delete=models.CASCADE
    )
