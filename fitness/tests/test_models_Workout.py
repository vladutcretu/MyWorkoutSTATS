...
class WorkoutModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.workout_exercise = WorkoutExercise.objects.create(
            workout=self.workout, exercise=self.exercise1, order=1
        )
    ...
    def test_exercise_is_in_workout(self):
        ...
        """
        self.assertTrue(
            self.workout.exercises.filter(id=self.exercise1.id).exists()
        )
    ...
    def test_exercise_is_not_in_workout(self):
        ...
        """
        self.assertFalse(
            self.workout.exercises.filter(id=self.exercise2.id).exists()
        )
