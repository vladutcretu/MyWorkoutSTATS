...
from fitness.models import MuscleGroup, Exercise, Workout, WorkingSet, WorkoutExercise
...
class WorkingSetModelTest(TestCase):
    ...
    def test_str_representation(self):
        ...
        expected_str = f"{self.exercise} - Set: {self.workingset.weight} weight, {self.workingset.repetitions} reps | {self.workingset.distance} meters, {self.workingset.time} minutes"
        self.assertEqual(str(self.workingset), expected_str)

    def test_delete_user_then_deletes_workingsets(self):
        """Test if after a user is deleted his working sets will be deleted as well"""
    ...
    def test_delete_workout_then_deletes_workingsets(self):
        """Test if after a workout is deleted his working sets will be deleted as well"""
        self.assertEqual(WorkingSet.objects.count(), 1)
        self.workout.delete()
        self.assertEqual(WorkingSet.objects.count(), 0)
    
    def test_delete_musclegroup_then_deletes_workingsets(self):
        """Test if after a musclegroup is deleted his working sets will be deleted as well"""
        self.assertEqual(WorkingSet.objects.count(), 1)
        self.musclegroup.delete()
        self.assertEqual(WorkingSet.objects.count(), 0)

    def test_delete_exercise_then_deletes_workingsets(self):
        """Test if after a exercise is deleted his working sets will be deleted as well"""
        self.assertEqual(WorkingSet.objects.count(), 1)
