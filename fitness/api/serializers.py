...
class ExerciseSerializer(serializers.ModelSerializer):
    ...
    musclegroup_id = serializers.IntegerField(write_only=True) # Accept musclegroup name in POST
    musclegroup_details = MuscleGroupSerializer(read_only=True, source='musclegroup')  # Return full musclegroup details in GET

    class Meta:
        model = Exercise
        fields = ['id', 'name', 'musclegroup_id', 'musclegroup_details', 'created']
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            raise serializers.ValidationError({"musclegroup_id": "User does not have the specified muscle group. Please specify an existing muscle group ID!"})
...
class WorkoutSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Workout
        fields = ['id', 'name', 'bodyweight', 'note', 'public', 'updated', 'created']

    def get_fields(self):
        """Hide`created` field on PUT/PATCH methods, but still show it on POST method."""
    ...
    def validate(self, attrs):
        ...
        if existing_workouts.exists():
            raise serializers.ValidationError({"workout": "You already have a workout created for this date. Try another date or delete the existing workout!"})
        return attrs
...
class WorkingSetSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = WorkingSet
        fields = ['id', 'workout_id', 'workout_details', 'exercise_id', 'exercise_details', 'type', 'weight', 'repetitions', 'distance', 'time', 'updated', 'created']
    ...
    def validate(self, attrs):
        ...
        # Check if workout and exercise are related (exercise is not added to the workout)
...
class DetailedWorkingSetSerializer(serializers.ModelSerializer):
    """Detailed version of working set serializer to use it for Detailed workout serializer."""
    class Meta:
        model = WorkingSet
...
class DetailedExerciseSerializer(serializers.ModelSerializer):
    """Detailed version of exercise serializer to use it for Detailed workout serializer."""
    ...
    def get_sets(self, obj):
        ...
        sets = WorkingSet.objects.filter(workout_id=workout_id, exercise=obj.exercise)  # Get sets based on the exercise
        return DetailedWorkingSetSerializer(sets, many=True).data


class DetailedWorkoutSerializer(serializers.ModelSerializer):
    """Detailed version of workout serializer to use it for Detailed workout API view."""
    exercises = serializers.SerializerMethodField()

    class Meta:
        model = Workout
        fields = ['id', 'name', 'created', 'updated', 'bodyweight', 'note', 'public', 'exercises']

    def get_exercises(self, obj):
        """Get all exercises associated with this workout, alongside with their sets."""
        workout_exercises = WorkoutExercise.objects.filter(workout=obj).select_related('exercise')  # Optimized filter with select_related
        serializer = DetailedExerciseSerializer(workout_exercises, many=True, context={'workout_id': obj.id})
