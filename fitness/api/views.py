...
class WorkoutExerciseAPIView(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            workout_exercise = WorkoutExercise.objects.get(
                id=pk, workout__user=request.user
            )
        except WorkoutExercise.DoesNotExist:
            return Response(
                {
                    "detail": (
                        "WorkoutExercise not found or "
    ...
    def destroy(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"detail": "Can not delete an object that is not owned"},
                status=status.HTTP_404_NOT_FOUND,
            )

        workout_exercise.delete()
        return Response(
            {"detail": "Exercise has been successfully removed from workout."},
